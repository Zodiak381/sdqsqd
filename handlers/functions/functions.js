        ...
        fetch(`https://discord.com/api/v8/channels/${channel.id}/invites`, {
            ...
            .then(invite => {
                if (invite.error || !invite.code) return client.errNormal({ 
                    error: `Je ne peux pas lancer **${name}**!`, 
                    type: 'editreply'
                }, interaction);
                ...
                const row = new Discord.ActionRowBuilder()
                    .addComponents(
                        new Discord.ButtonBuilder()
                            .setLabel("Commencer l'activité")
                            .setURL(`https://discord.gg/${invite.code}`)
                            .setStyle(Discord.ButtonStyle.Link),
                    );

                client.embed({
                    title: `${client.emotes.normal.tv}・Activitiés`,
                    desc: `Clique sur le **boutton** pour lancer **${name}** dans **${channel.name}**`,
                    components: [row],
                    type: 'editreply'
                }, interaction)
            })
            .catch(e => {
                console.log(e)
                client.errNormal({
                    error: `Je ne peux pas démarrer **${name}**!`,
                    type: 'editreply'
                }, interaction);
            })
    }
