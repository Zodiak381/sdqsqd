...
module.exports = (client) => {
    ...
    if (client.shard.ids[0] === 0) console.log(chalk.blue(chalk.bold(`System`)), (chalk.white(`>>`)), (chalk.green(`Chargelent des commandes`)), (chalk.white(`...`)))
    ...
    fs.readdirSync('./src/interactions').forEach(dirs => {
        ...
        if (client.shard.ids[0] === 0) console.log(chalk.blue(chalk.bold(`System`)), (chalk.white(`>>`)), chalk.red(`${commandFiles.length}`), (chalk.green(`commandes`)), chalk.red(`${dirs}`), (chalk.green(`chargées`)));
    ...
    (async () => {
        try {
            const embed = new Discord.EmbedBuilder()
                .setDescription(`Lancement et Rechargement des (/) commandes.`)
            ...
            const embedFinal = new Discord.EmbedBuilder()
                .setDescription(`${commands.length} commandes (/) rechargées.`)
                .setColor(client.config.colors.normal)
            interactionLogs.send({
                username: 'Bot Logs',
                embeds: [embedFinal]
