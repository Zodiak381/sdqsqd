        ...
        Schema.findOne({ Guild: message.guild.id }, async (err, data) => {
            if (data) {
                if (data.AntiInvite == true) {
                    ...
                    if (code || content.includes('discord.gg/')) {
                        Schema2.findOne({ Guild: message.guild.id }, async (err, data2) => {
                            if (data2) {
                                ...
                                client.embed({
                                    title: `${client.emotes.normal.error}・Modération`,
                                    desc: `Les liens discord ne sont pas autoriser dans ce serveur!`,
                                ...
                                client.embed({
                                    title: `${client.emotes.normal.error}・Moderation`,
                                    desc: `Les liens discord ne sont pas autoriser dans ce serveur!`,
                                    color: client.config.colors.error,
                                    content: `${message.author}`
                                }, message.channel)
                    ...
                    if (content.includes('http://') || content.includes('https://') || content.includes('www.')) {
                        Schema2.findOne({ Guild: message.guild.id }, async (err, data2) => {
                            if (data2) {
                                ...
                                client.embed({
                                    title: `${client.emotes.normal.error}・Moderation`,
                                    desc: `Les liens ne sont pas autoriser dans ce serveur!`,
                                    color: client.config.colors.error,
                                    content: `${message.author}`
                                }, message.channel)
                            }
                        ...
                        Schema2.findOne({ Guild: newMessage.guild.id }, async (err, data2) => {
                            if (data2) {
                                ...
                                let error = new Discord.EmbedBuilder()
                                    .setTitle(`${client.emotes.normal.error}・Moderation`)
                                    .setAuthor(client.user.username, client.user.avatarURL())
                                    .setDescription(`Les liens discord ne sont pas autoriser dans ce serveur!`)
                        ...
                        Schema2.findOne({ Guild: newMessage.guild.id }, async (err, data2) => {
                            if (data2) {
                                ...
                                var error = new Discord.EmbedBuilder()
                                    .setTitle(`${client.emotes.normal.error}・Moderation`)
                                    .setAuthor(client.user.username, client.user.avatarURL())
                                    .setDescription(`Les liens ne sont pas autoriser dans ce serveur!`)
                                    .setColor(client.config.colors.error)
                                    .setFooter({ text: client.config.discord.footer })
                                    .setTimestamp();
