        ...
        if (messageData && messageData.inviteJoin) {
            ...
            welcomeSchema.findOne({ Guild: member.guild.id }, async (err, channelData) => {
                if (channelData) {
                    ...
                    client.embed({
                        title: `👋・Welcome`,
                        desc: `Je ne peux pas dire qui a invité **${member} | ${member.user.tag}**`
                    }, channel).catch(() => { })
                }
            })
        }
            ...
            if (messageData) {
                ...
                welcomeSchema.findOne({ Guild: member.guild.id }, async (err, channelData) => {
                    if (channelData) {
                        ...
                        client.embed({
                            title: `👋・Welcome`,
                            desc: `**${member} | ${member.user.tag}** a été invité par ${inviter.tag} **(${data.Invites} invitations)**`
                        }, channel)
                    }
                })
            }
            ...
            if (messageData) {
                ...
                welcomeSchema.findOne({ Guild: member.guild.id }, async (err, channelData) => {
                    if (channelData) {
                        ...
                        await client.embed({
                            title: `👋・Welcome`,
                            desc: `**${member} | ${member.user.tag}** a été invité par ${inviter.tag} **(1 invite)**`
                        }, channel).catch(() => { })
                    }
                })
            }
