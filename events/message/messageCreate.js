...
module.exports = async (client, message) => {
  ...
  if (message.channel.type === Discord.ChannelType.DM) {
    let embedLogs = new Discord.EmbedBuilder()
      .setTitle(`💬・Nouveau Message MP!`)
      .setDescription(`Le bot a reçu un message MP!`)
      .addFields(
        { name: "👤┆Envoyé par", value: `${message.author} (${message.author.tag})`, inline: true },
        { name: `💬┆Message`, value: `${message.content || "Aucun"}`, inline: true },
      )
    ...
    if (message.attachments.size > 0)
      embedLogs.addFields(
        { name: `📃┆Image`, value: `${message.attachments.first()?.url}`, inline: false },
      )
    return dmlog.send({
      username: "Bot DM",
      embeds: [embedLogs],
  ...
  if (
    ...
    client
      .embed(
        {
          title: "Salut !",
          desc: `Utilise les commandes avec les ${client.emotes.normal.slash} slashs`,
          fields: [
            {
              name: "📨┆Invite moi",
              value: `Invite le bot sur ton Serveur! [Clique Ici](${client.config.discord.botInvite})`,
            },
            {
              name: "❓┇Je n'ai aucun slash",
              value:
                "Le bot ne doit pas avoir les permissions de les mettres, vérifie ses permissions",
            },
            {
              name: "❓┆Besoin d'aide?",
              value: `Pour des question, rejoint le [serveur support](${client.config.discord.serverInvite})!`,
            },
            {
              name: "🐞┆Bug trouvé?",
              value: `Report tous les bugs avec: \`/report bug\`!`,
            },
          ],
          components: [row],
        },
  ...
  if (cmdx) {
    if (cmdx.Action == "Normal") {
      ...
      return message.author.send({ content: cmdx.Responce }).catch((e) => {
        client.errNormal(
          {
            error: "Je ne peux pas t'envoyer de MP!!",
          },
          message.channel
        );
      });
  ...
  if (command) {
    ...
    client.embed(
      {
        title: "👋・Salut",
        desc: `Le bot utilise totalement les ${client.emotes.normal.slash} slashs. Les commandes messages ont expirés! Essaie de rendre ton serveur meilleur avec moi!`,
        fields: [
          {
            name: "❓┇Je ne vois aucune slash",
            value:
              "Le bot ne dois pas avoir les permissions d'ajouter des application sur ton serveur",
          },
        ],
        components: [row],
      },
