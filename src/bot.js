...
process.on('unhandledRejection', error => {
    ...
    if (error) if (error.length > 950) error = error.slice(0, 950) + '... regarde la console pour plus de détails';
    if (error.stack) if (error.stack.length > 950) error.stack = error.stack.slice(0, 950) + '... regarde la console pour plus de détails';
    if(!error.stack) return
    const embed = new Discord.EmbedBuilder()
        .setTitle(`🚨・Rejet d'une Promise non gérée`)
        .addFields([
            {
                name: "Error",
