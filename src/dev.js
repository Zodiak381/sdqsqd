...
if(!process.argv[2]) {
    console.log(chalk.red(`[ERROR]`), chalk.white(`>>`), chalk.red(`Developer Badge`), chalk.white(`>>`), chalk.red(`Entre l'id d'un membre!`))
    process.exit(1);
}
// Find the user
model.findOne({
    ...
    if(!data) {
        ...
        newData.save();
        console.log((chalk.white(`>>`)), chalk.red(`Developer Badge`), chalk.green(`a été ajouté au membre!`))
