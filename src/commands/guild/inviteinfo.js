...
module.exports = async (client, interaction, args) => {
  ...
  const verifyFlags = {
    ...
    1 : `Doit avoir un email vérifié sur le compte`,
    2 : `Il faut être enregistré sur Discord depuis plus de 5 minutes`,
    3 : `Doit être un membre du serveur depuis plus de 10 minutes`,
  ...
  axios.get(`https://discord.com/api/v9/invites/${encodeURIComponent(invite)}`).catch(async () => {
    return client.errNormal({
      error: "Je ne peux pas trouver le serveur",
      type: 'editreply'
    }, interaction)
  }).then(async (raw) => {
    const { data } = raw;
    ...
    return client.embed({
      title: `📨・Informations de l'Invitations`,
      ...
      fields: [
        ...
        },
        {
          ...
          value: `${data.guild.icon && data.guild.banner && data.guild.splash ? `` : `Aucune information`}
          ${data.guild.icon ? `[Photo du Serveur](https://cdn.discordapp.com/icons/${data.guild.id}/${data.guild.icon}.png?size=4096)` : ``}
