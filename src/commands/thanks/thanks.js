        ...
        if (data) {
            ...
            thanksSchema.findOne({ User: target.id }, async (err, data) => {
                if (data) {
                    data.Received += 1;
                    data.save();
                    client.succNormal({ text: `Tu as remercié <@${target.id}>! Il a maintenant \`${data.Received}\` remerciments`, type: 'editreply' }, interaction);
                }
                else {
                    new thanksSchema({
                        User: target.id,
                    ...
                    client.succNormal({ text: `Tu as remercié <@${target.id}>! Il a maintenant \`1\` remerciments`, type: 'editreply' }, interaction);
                }
            })
