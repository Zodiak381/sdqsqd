        ...
        if (data) {
            ...
            if (interaction.guild.channels.cache.find(c => c.id === ticketCategory.id)) {
                client.users.fetch(ticketData.creator).then(async usr => {
                    ...
                    try {
                        var closeMessageTicket = "Voici la transcription de votre billet, veuillez la conserver si vous souhaitez vous y référer un jour.!";
