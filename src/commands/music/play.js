...
module.exports = async (client, interaction, args) => {
    ...
    if (!player) {
        ...
        if (!channel.joinable) return client.errNormal({
            error: `Le salon n'est pas joignable`,
            type: 'editreply'
        }, interaction);
        player.connect()
    ...
    if (res.loadType === 'LOAD_FAILED') {
        ...
        return client.errNormal({
            error: `Erreur lors de l'obtention de la musique.. Ressaie plus tard`,
            type: 'editreply'
        }, interaction);
    }
    ...
    switch (res.loadType) {
        ...
        case 'TRACK_LOADED': {
            ...
            if (!player.playing && !player.paused) {
                ...
                client.embed({
                    ...
                    url: track.uri,
                    desc: `La chanson a été ajoutée à la queue!`,
        ...
        case 'SEARCH_RESULT': {
            ...
            client.embed({
                ...
                fields: [
                    {
                        name: `❓┆Annuler la Recherche?`,
                        value: `Ecrit \`annuler\` pour arrêter la recherche`,
                        inline: true
                    }
                ],
                components: [row, row2],
            ...
            try {
                ...
                if (!player.queue.current) player.destroy();
                return client.errNormal({
                    error: `Tu n'as pas donner de séléction`,
                    type: 'editreply'
                }, interaction)
            }
            ...
            if (first.toLowerCase() === 'annuler') {
                if (!player.queue.current) player.destroy();
                return interaction.channel.send('Séléction annulée.');
            }

            const index = Number(first) - 1;
            if (index < 0 || index > max - 1) return client.errNormal({
                error: `Le chiffre que tu as entré est trop grand ou trop petit (1-${max})`,
                type: 'editreply'
            }, interaction)
